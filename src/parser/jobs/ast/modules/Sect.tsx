...
// Fabricated ability definitions for Sects
...
// Just in case they go through a whole fight with no sect and no aspected abilities, we're not going to babysit the whole log
...
export default class Sect extends Module {
	...
	normalise(events: any) {
		...
		for (const event of events) {
			...
			if (!aspectedCast && event.type === 'cast'
				&& ASPECTED_ACTIONS.includes(event.ability.guid)) {
				// Detecting a cast of an aspected action, so now we examine what comes out of it
				aspectedCast = event

			} else if (aspectedCast && event.type === 'applybuff' && (DIURNAL_SECT_STATUSES.includes(event.ability.guid)
				...
				if (this.mapAspectedCastToBuff(aspectedCast.ability.guid).includes(event.ability.guid)
	...
	private _onApplySect(event: BuffEvent) {
		if (!this._sectId) {
			const sect = getDataBy(ACTIONS, 'id', this.mapSectBuffToCast(event.ability.guid))
			if (sect) {
				this._sectId = sect.id
			}
		}
	...
	/**
	 * Accepts an aspected cast action and returns the possible statuses that come from it
	 */
	public mapAspectedCastToBuff(actionId: number) {
	...
	/**
	 * Accepts a Sect status and returns the matching Sect action ID
	 */
	public mapSectBuffToCast(statusId: number) {
		switch (statusId) {
